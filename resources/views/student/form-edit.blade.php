                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <div class="card-title">Ubah Data Siswa</div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <form action="{{ route('student.update', $student->id) }}" method="POST">
                                    ...
                                    <div class="form-group mb-2">
                                        ...
                                        <select name="classroom_id" id="classroom" class="form-select">
                                            ...
                                            @foreach ($classroom as $item)
                                                <option {{ $item->id == $student->classroom_id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name ?? 'tidak ada data kelas' }}</option>
                                            @endforeach
                                        </select>
                                        @error('classroom')
                                            <small class="text-danger">{{ $message }}</small>
                                    ...
                                    <button class="btn btn-success btn-sm mx-2">Ubah</button>
                                </form>
                            </div>
                        </div>
                    </div>
